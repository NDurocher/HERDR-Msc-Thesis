...
from metrics_utils import (add2pickle, plot_action_cam_view, plot_actions, plot_heatmap,
                           plot_trajectory)
...
class HERDRenv():
    ...
    def new_run(self, action_hist, im_hist, GND_hist, folder):
        ...
        self.H5File = h5py.File(f'{dir_name}/old_carla_hdf5s/{folder}/{self.H5File_name}.h5', 'a')
    ...
    def cleanup(self):
        ...
        self.client.apply_batch_sync([carla.command.DestroyActor(x.id) for x in self.controller_list])
        self.world.tick()
        print('Done.')
    ...
    def get_spawn_ORCA(self, side=0, num_spots=20):
        if side == 1:
            ...
            y = np.random.choice(np.linspace(self.spawn_locations[:,1].max(),self.spawn_locations[:,1].min(),num_spots))
    ...
    def pop_peds_ORCA(self, num_peds):
        self.load_spawns(f'/home/nathan/HERDR/spawn_locations_Test_Orca2.txt')
        side1_list = []
        side2_list = []
        num_spots = int((self.spawn_locations[:,1].max() - self.spawn_locations[:,1].min()) / 0.5)
        for i in range(num_peds):
...
def test(env, Herdr, blk):
    ...
    try:
        ...
        while not Herdr.done:
            ...
            if len(Herdr.collision_hist) > 0:
                Herdr.collision_locs.append(location2tensor(Herdr.vehicle.get_location()))
                break
        ...
        '# of Peds': num_peds, 'Sim Time': int(save_frame_count/5)}
        print(data)
...
def check_spawn_points():
    ...
    debug.draw_arrow(arrow, arrow_top, life_time=30.)
...
def ORCA_Test(env, agent):
    ...
    try:
        ...
        tau = 2
        dt = 1/5
        agent_spawn = carla.Transform(carla.Location(x=float(env.spawn_locations[:,0].min()), y=-109, z=0.177637), rotation=carla.Rotation(yaw=0.))
        agent_spawn.location.y = np.random.choice(np.linspace(env.spawn_locations[:,1].min(),env.spawn_locations[:,1].max(),4))
        agent_goal = carla.Transform(carla.Location(x=float(env.spawn_locations[:,0].max()), y=np.random.choice(np.linspace(env.spawn_locations[:,1].min(),env.spawn_locations[:,1].max(),2)), z=0.177637))
        # debug = world.debug
        ...
        env.orca_actor_list.append(ORCAAgent(agent.vehicle, agent_goal.location,radius=0.5, max_speed=1.4))
        ...
        collision_object = 'None' if len(agent.collision_hist) == 0 else agent.collision_hist[-1]
        data = {'Times in ped space': env.ped_space_count, 'Path length': pl, 
        'Avg min dist to ped': np.asarray(env.dist2peds_list).mean(), 'Success': agent.success, 'Collision Object': collision_object,
        'Point2Point Dist': agent.p2pdist.item(), 'Agent goal gain': agent.goal_gain, 'Agent action gain': agent.action_gain,
        '# of Peds': num_peds, 'Sim Time': int(save_frame_count/5), 'Weather': weather}
        agent.cleanup()
        # try:
        #     job.join()
        #     job.close()
...
if __name__ == '__main__':
    ...
    agent = Herdragent(training=True, model_name=f'{model_name}.pth') 
    # agent = Pedagent()
    # len_df = 0
    # while len_df < 1:
