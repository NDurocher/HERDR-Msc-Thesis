...
class HERDRenv():
    ...
    def reset(self):
        ...
        self.world.set_weather(dict_WP[preset_list[random.randint(0,11)]])
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                run_data = carla_hdf5dataclass(f"/home/nathan/HERDR/all_carla_hdf5s/{recording_data}/{env.H5File_name}.h5", Herdr.horizon, imagefile_path=f"/home/nathan/HERDR/carla_images/{recording_data}/", load_all_files=False)
...
def test():
    ...
    env.world.set_weather(dict_WP[preset_list[6]])
    blk = 2
    Herdr = Herdragent(training=False, model_name='carla07-04-2022--14:41.pth',test_block=blk) # 'carla02-04-2022--17:42.pth'
    try:
        # env.reworld()
        # env.reset()
        env.pop_map_pedestrians(num_peds=20, test_block=blk)
        Herdr.reset()
        env.set_recordings()
        job = Process(target=env.background_save, args=(env.plot_hist_front,env.plot_hist_top,), daemon=True)
        job.start()
        start_time = time.time()
        save_frame_count = 0
        while not Herdr.done | (time.time() - start_time >= 100):
            Herdr.step()
            env.world.tick()
            env.dist2peds(Herdr)
            Herdr.is_safe(env.controller_list)
