...
class HERDRenv():
    ...
    def new_run(self):
        ...
        self.d1 = self.H5File.create_dataset(f"{self.now}/actions", data=self.action_hist)
        self.d2 = self.H5File.create_dataset(f"{self.now}/gnd", data=self.GND_hist)
        maxlen = len(str(self.now))
        dtipe = 'S{0}'.format(maxlen)
        self.d3 = self.H5File.create_dataset(f"{self.now}/img", data=self.im_hist, dtype=dtipe)
...
def main():
    ...
    for i in range(10):
        # if (i % 3) == 0:
            # H5File_name = f'{datetime.now()}'
        Herdr = HERDRenv(training=False)
        Herdr.H5File_name = H5File_name
        try:
            Herdr.reworld()
            Herdr.reset()
            Herdr.pop_map_pedestrians(num_peds=75)
            Herdr.set_recordings()
            job = Process(target=Herdr.background_save, args=(Herdr.plot_hist_front,Herdr.plot_hist_top,), daemon=True)
            job.start()
            print(f'--- START Run {i+1} ---')
            start_time = time.time()
            save_frame_count = 0
            while not Herdr.done | (time.time() - start_time >= 30):
                Herdr.step()
                if save_frame_count % 3 == 0:
                    Herdr.get_recordings()
                Herdr.world.wait_for_tick()
                Herdr.is_safe()
            ...
            Herdr.done = True
            Herdr.plot_hist_front.put('done')
            ...
            Herdr.new_run()
            Herdr.H5File.close()
            Herdr.top_writer.finish()
            Herdr.writer.finish()
            plot_trajectory(np.asarray(Herdr.pos_hist), torch.ones((len(Herdr.pos_hist))), Herdr.GOAL[0,0,:], collision=len(Herdr.collision_hist))
            plt.savefig('./trajectory.png')
            print(f'Run time: {time.time()-start_time:.4f}')

        finally:
            print('Destroying actors')
            Herdr.client.apply_batch([carla.command.DestroyActor(x.id) for x in Herdr.actor_list])
            print('Destroying Sensors')
            [x.stop() for x in Herdr.sensor_list]
            Herdr.client.apply_batch([carla.command.DestroyActor(x.id) for x in Herdr.sensor_list])
            print('Destroying controllers')
            [x.stop() for x in Herdr.controller_list]
            Herdr.client.apply_batch([carla.command.DestroyActor(x.id) for x in Herdr.controller_list])
            print('Done.')
        del Herdr
        time.sleep(2)
